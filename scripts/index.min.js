...
addLicence(){let date=new Date();let year=date.toLocaleDateString().slice(6);this.footer.textContent=`© Made by Gup@lyuk 2023 - ${year}`;}}
...
module.exports=IndexCharts;},{}],10:[function(require,module,exports){class Login{constructor(){this.body=document.querySelector('body');this.cartBtn=document.querySelector('.login-btn');this.formWindow=document.querySelector('.login-window');this.closeOrderBtn=document.querySelector('.order-close');this.orderWindowBody=document.querySelector('.order-window__data');this.orderWindowProducts=document.querySelector('.order-window__products');this.clearBtn=document.querySelector('.order-clear-btn');this.calculationBtn=document.querySelector('.order-count-btn');this.sendBtn=document.querySelector('.order-send-btn');this.orderElements=null;this.allElements=null;this.choosedElements=[];this.finalProductsArr=[];this.finalSumm=null;this.finalCount=null;}
...
class Product{constructor(id,article,code,name,count,priceTT,priceOpt,priceBigOpt){this.id=id;this.article=article;this.code=code;this.name=name;this.count=count;this.priceTT=priceTT;this.priceOpt=priceOpt;this.priceBigOpt=priceBigOpt;}}
for(let i=0;i<this.orderElements.length;i++){for(let j=0;j<this.allElements.length;j++){if(this.allElements[j].Id==this.orderElements[i]){this.choosedElements.push(new Product(this.allElements[j].Id,this.allElements[j].Article,this.allElements[j].Code,this.allElements[j].Name,1,this.allElements[j].PriceTT,this.allElements[j].PriceOpt,this.allElements[j].PriceBigOpt,))}}}
this.formWindow.style.display='flex';this.body.style.height='100dvh';this.body.style.overflow='hidden';productsBody.append(...this.choosedElements.map(product=>this.createElements(product,this.finalProductsArr)));}
hideWindow(){this.formWindow.style.display='none';this.body.style.height='100%';this.body.style.overflow='';this.choosedElements=[];this.sendBtn.classList.add('disabled');}
createElements(product,finalProduct){let prod=document.createElement('div');prod.classList.add('product');prod.dataset.index=product.id;let close=document.createElement('div');close.classList.add('product__close');let icon=document.createElement('i');icon.classList.add('small');icon.classList.add('material-icons');icon.classList.add('mat-close');icon.dataset.index=product.id;icon.append('delete_forever');close.dataset.index=product.id;close.append(icon);let photo=document.createElement('img');photo.classList.add('product__image');let checkArticle=product.article;let result;let findSlash=checkArticle.indexOf('/');if(findSlash!=-1){let check=checkArticle.split('');let final;let find=check.indexOf('/');check[find]='-';final=[...check];result=final.join('');checkArticle=result;}
photo.setAttribute('src',`./images/photo-all/images/${checkArticle}.webp`);let code=document.createElement('div');code.classList.add('product__code');code.append(product.code);let name=document.createElement('div');name.classList.add('product__name');name.append(product.name);let count=document.createElement('input');count.type='number';if(finalProduct.length==0){count.value=0;}else if(finalProduct.length>0){for(let i=0;i<finalProduct.length;i++){if(finalProduct[i].ind==product.id){count.value=finalProduct[i].cnt;}}}
count.dataset.index=product.id;count.classList.add('product__count');let price=document.createElement('div');price.classList.add('product__price');if(finalProduct.length==0){price.append('-');}else if(finalProduct.length>0){for(let i=0;i<finalProduct.length;i++){if(finalProduct[i].ind==product.id){price.append(finalProduct[i].prc);}}}
...
calculate(){let showSumm=document.querySelector('.summ__result');let showCount=document.querySelector('.count__result');let radios=document.querySelectorAll('input[type="radio"]');let checkedPrice=null;for(let radio of radios){if(radio.checked){checkedPrice=radio.value;}}
...
class CountObj{constructor(ind,cnt,prc,cd,ttl){this.ind=ind;this.cnt=cnt;this.prc=prc;this.cd=cd;this.ttl=ttl;}}
let allCounts=document.querySelectorAll('.product__count');this.finalProductsArr=[];if(checkedPrice=='priceTT'){for(let i=0;i<allCounts.length;i++){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceTT),this.choosedElements[i].code,this.choosedElements[i].name));}}else if(checkedPrice=='priceOpt'){for(let i=0;i<allCounts.length;i++){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceOpt),this.choosedElements[i].code,this.choosedElements[i].name));}}else if(checkedPrice=='priceBigOpt'){for(let i=0;i<allCounts.length;i++){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceBigOpt),this.choosedElements[i].code,this.choosedElements[i].name));}}
...
let chechFree=this.finalProductsArr.filter(elem=>elem.prc=='0');if(chechFree.length>0){M.toast({html:`Товары с 0 ценой!`,displayLength:1500,classes:'toast-red'});return}
if(client.value.trim()==''){M.toast({html:`Укажите клиента!`,displayLength:1500,classes:'toast-red'});return}
let confirmSend=confirm("Отправить заказ?");if(confirmSend){this.telegram(this.finalProductsArr,this.finalSumm,this.finalCount,client.value);Email.send({Host:"smtp.elasticemail.com",Username:"stanislav.gupalyuk@gmail.com",Password:"590FA512160A4E2AEEDA3DB7C409E7431DEC",To:'stanislav.gupalyuk@ukr.net',From:"stanislav.gupalyuk@gmail.com",Subject:`Заказ (${client.value.trim()} от ${nowDate} / ${nowTime})`,Body:table.outerHTML,}).then(M.toast({html:`Заказ отправлен!`,displayLength:1500,classes:'toast-green'})).then(this.sendBtn.classList.add('disabled')).then(setTimeout(()=>{location.reload()},1500));}}
clear(){let dellAll=confirm("Очистить всё?");if(dellAll){this.hideWindow();sessionStorage.setItem('inCart',[0]);location.reload();}}
telegram(productObj,fullSumm,fullCount,client){const TOKEN='6746536289:AAGZlKfEqPi_t_tlzj40psKkd3PHZit1g3E';const CHAT_ID='-1002021384272';const URI_API=`https://api.telegram.org/bot${TOKEN}/sendMessage`;let requestObj=productObj;let requestClient=client;let message=`<b>Distribution</b>\n`;message+=`<b>Клиент: </b> ${requestClient}\n`;message+=`<b>Всего(шт): </b> ${fullCount}\n`;message+=`<b>Сумма(грн): </b> ${fullSumm.toFixed(2)}\n`;message+=`-----\n`;requestObj.map(e=>createSendMessage(e));function createSendMessage(elem){message+=`<b>Код: </b> ${elem.cd}\n`;message+=`<b>Название: </b> ${elem.ttl}\n`;message+=`<b>Количество(шт): </b> ${elem.cnt}\n`;message+=`<b>Цена(грн): </b> ${elem.prc}\n`;message+=`-----\n`;}
