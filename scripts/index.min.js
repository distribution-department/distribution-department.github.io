...
getGoods(){this.disabledBtns();this.mainSectionBody.style.boxShadow='none';$.ajax({url:"https://script.google.com/macros/s/AKfycbw_TqaALIrZH_xIYVdB4ptDgRE-ScT5HH__9yOOa8wNqRE4VTLKhe6ZOQxryp89L-1O/exec",method:"get",dataType:"json",success:(data)=>{setTimeout(()=>{this.elementSection.innerHTML='';sessionStorage.setItem('data',JSON.stringify(data.goods));sessionStorage.setItem('current','fifty');let response=data.goods.reverse();let temporaryGoods=[];for(let i=19;i>=0;i--){temporaryGoods.push(response[i])}
