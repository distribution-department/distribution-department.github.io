...
let title=`Всего ${dataArr.length} позиций`;const data={labels:grope,datasets:[{label:title,data:count,backgroundColor:['rgba(255, 99, 132, 0.2)','rgba(255, 159, 64, 0.2)','rgba(255, 205, 86, 0.2)','rgba(75, 192, 192, 0.2)','rgba(54, 162, 235, 0.2)','rgba(153, 102, 255, 0.2)','rgba(201, 203, 207, 0.2)','rgba(255, 99, 132, 0.2)','rgba(255, 159, 64, 0.2)','rgba(255, 205, 86, 0.2)','rgba(75, 192, 192, 0.2)','rgba(54, 162, 235, 0.2)','rgba(153, 102, 255, 0.2)','rgba(201, 203, 207, 0.2)','rgba(255, 99, 132, 0.2)','rgba(255, 159, 64, 0.2)','rgba(255, 205, 86, 0.2)','rgba(75, 192, 192, 0.2)','rgba(54, 162, 235, 0.2)','rgba(153, 102, 255, 0.2)','rgba(201, 203, 207, 0.2)','rgba(255, 99, 132, 0.2)','rgba(255, 159, 64, 0.2)','rgba(255, 205, 86, 0.2)','rgba(75, 192, 192, 0.2)',],borderColor:['rgb(255, 99, 132)','rgb(255, 159, 64)','rgb(255, 205, 86)','rgb(75, 192, 192)','rgb(54, 162, 235)','rgb(153, 102, 255)','rgb(201, 203, 207)','rgb(255, 99, 132)','rgb(255, 159, 64)','rgb(255, 205, 86)','rgb(75, 192, 192)','rgb(54, 162, 235)','rgb(153, 102, 255)','rgb(201, 203, 207)','rgb(255, 99, 132)','rgb(255, 159, 64)','rgb(255, 205, 86)','rgb(75, 192, 192)','rgb(54, 162, 235)','rgb(153, 102, 255)','rgb(201, 203, 207)','rgb(255, 99, 132)','rgb(255, 159, 64)','rgb(255, 205, 86)','rgb(75, 192, 192)',],borderWidth:1}]};const config={type:'bar',data:data,options:{plugins:{legend:{labels:{font:{}}},subtitle:{display:true,text:'Товар в разрезе категорий',font:{weight:'bold'}}},scales:{y:{beginAtZero:true}}},};myChartAll=new Chart(document.getElementById('myChartAll'),config);});tmBtn.addEventListener('click',()=>{myChartAll.destroy();let dataArr=[...JSON.parse(sessionStorage.getItem('data'))];let gropeNames=['Krauff','Helfer','Oselya','LAV','Keramia','Ritter','Martex','Miorro','Mitsui','Libbey','Viola','Lorenzzo','Hostel','Lotti'];let finalData=[];for(let i=0;i<gropeNames.length;i++){finalData.push({tm:gropeNames[i],count:dataArr.filter(e=>e.TradeMark==gropeNames[i]).length});}
...
showWindow(){this.orderElements=JSON.parse(sessionStorage.getItem('inCart'));let productsBody=document.querySelector('.order-window__products');productsBody.innerHTML='';if(this.orderElements==0){M.toast({html:`Корзина пуста!`,displayLength:1500,classes:'toast-red'});return}
...
if(checkedPrice==null){M.toast({html:`Выберите тип цены!`,displayLength:1500,classes:'toast-red'});this.sendBtn.classList.add('disabled');return}
let inputCount=document.querySelectorAll('input[type="number"]');for(let num of inputCount){if(num.value<1){M.toast({html:`Укажите везде количество!`,displayLength:1500,classes:'toast-red'});this.sendBtn.classList.add('disabled');return}}
...
let chechFree=this.finalProductsArr.filter(elem=>elem.prc=='0');if(chechFree.length>0){M.toast({html:`Есть товар без цены!`,displayLength:1500,classes:'toast-red'});return}
if(this.isContain.length>0){M.toast({html:`Есть товар без остатка!`,displayLength:1500,classes:'toast-red'});return}
if(payType.val()==''){M.toast({html:`Не указан тип оплаты!`,displayLength:1500,classes:'toast-red'});return}
if(client.value.trim()==''){M.toast({html:`Укажите клиента!`,displayLength:1500,classes:'toast-red'});return}
let confirmSend=prompt('Введите код подтверждения...');if(confirmSend.trim()==this.getConfY){this.sendBtn.classList.add('disabled');this.calculationBtn.classList.add('disabled');this.clearBtn.classList.add('disabled');this.cartBtn.classList.add('disabled');this.closeOrderBtn.classList.add('disabled');this.calcBtn.classList.add('disabled');this.telegram(this.finalProductsArr,this.finalSumm,this.finalCount,client.value,typeOfPayTelegram,clientCommentTelegram);Email.send({Host:"smtp.elasticemail.com",Username:"stanislav.gupalyuk@gmail.com",Password:"590FA512160A4E2AEEDA3DB7C409E7431DEC",To:['parhomenko@tnp.com.ua','gupalyuk@tnp.com.ua'],From:"stanislav.gupalyuk@gmail.com",Subject:`Заказ (${client.value.trim()} от ${nowDate} / ${nowTime})`,Body:message.outerHTML,}).then(this.emailSend()).then(setTimeout(()=>{location.reload()},2500));}else{alert('Неверный код!');return;}}
clear(){let dellAll=confirm("Очистить всё?");if(dellAll){sessionStorage.setItem('inCart',[0]);location.reload();}}
emailSend(){this.formWindow.style.display='none';this.bodyData.style.display='none';this.bodyMainChart.style.display='none';this.bodyDonatChart.style.display='none';this.sendEmailWindow.style.display='block';this.sendEmailWindow.style.maxWidth='500px';this.sendEmailWindow.style.maxHeight='500px';this.sendEmailText.style.display='block';}
...
this.removeDisabledBtns();this.search.value='';},100);let personalCart=document.querySelector('.item__cart-summary');personalCart.addEventListener('click',()=>{let allInCart=JSON.parse(sessionStorage.getItem('inCart'));if(allInCart=='0'){allInCart=[currentIndex]}
