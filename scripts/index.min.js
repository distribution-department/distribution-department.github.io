...
module.exports=IndexCharts;},{}],10:[function(require,module,exports){class Login{constructor(){this.mainSection=document.querySelector('.main-section');this.cartBtn=document.querySelector('.enter-cart-btn');this.closeOrderBtn=document.querySelector('.back-cart-btn');this.calcBtn=document.querySelector('.calculate-btn');this.formWindow=document.querySelector('.login-window');this.orderWindowBody=document.querySelector('.order-window__data');this.orderWindowProducts=document.querySelector('.order-window__products');this.bodyData=document.querySelector('.main-section__body');this.bodyMainChart=document.querySelector('.chart-all');this.bodyDonatChart=document.querySelector('.chart-new');this.sendEmailWindow=document.querySelector('.send-window');this.sendEmailText=document.querySelector('.send-window-text');this.clearBtn=document.querySelector('.order-clear-btn');this.calculationBtn=document.querySelector('.order-count-btn');this.sendBtn=document.querySelector('.order-send-btn');this.orderElements=null;this.allElements=null;this.choosedElements=[];this.finalProductsArr=[];this.finalSumm=null;this.finalCount=null;this.isContain=[];let today=new Date();this.getConfY=today.getFullYear();}
init(){this.allElements=JSON.parse(sessionStorage.getItem('data'));this.sendBtn.classList.add('disabled');this.cartBtn.addEventListener('click',()=>this.showWindow());this.closeOrderBtn.addEventListener('click',()=>this.hideWindow());this.orderWindowBody.addEventListener('click',(e)=>this.dellElement(e));this.sendBtn.addEventListener('click',(e)=>this.send());this.calculationBtn.addEventListener('click',(e)=>this.calculate());this.clearBtn.addEventListener('click',(e)=>this.clear());}
...
typeOfPay.setAttribute('style','margin-bottom: 10px;');clientComment.setAttribute('style','margin-bottom: 10px;');message.append(typeOfPay);message.append(clientComment);message.append(table);let nowDate=new Date().toLocaleDateString();let nowTime=new Date().toLocaleTimeString();function createDataTableBody(info){let tableRow=document.createElement('tr');tableRow.setAttribute('style','border: 1px solid black; text-align: center; font-size: 15px;');let tableDataCode=document.createElement('td');tableDataCode.append(info.cd);tableDataCode.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableRow.append(tableDataCode);let tableDataName=document.createElement('td');tableDataName.append(info.ttl);tableDataName.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableRow.append(tableDataName);let tableDataCount=document.createElement('td');tableDataCount.append(info.cnt);tableDataCount.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableRow.append(tableDataCount);let tableDataPrice=document.createElement('td');tableDataPrice.append(info.prc.toString().replace('.',','));tableDataPrice.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableRow.append(tableDataPrice);return tableRow;}
...
if(client.value.trim()==''){Swal.fire({icon:"warning",title:"Укажите клиента!",showConfirmButton:false,timer:1500});return}
let confirmSend=prompt('Введите код подтверждения...');if(confirmSend.trim()==this.getConfY){this.sendBtn.classList.add('disabled');this.calculationBtn.classList.add('disabled');this.clearBtn.classList.add('disabled');this.cartBtn.classList.add('disabled');this.closeOrderBtn.classList.add('disabled');this.calcBtn.classList.add('disabled');this.telegram(this.finalProductsArr,this.finalSumm,this.finalCount,client.value,typeOfPayTelegram,clientCommentTelegram);this.copyToGoogleTable(client.value,typeOfPayTelegram,clientCommentTelegram);Email.send({Host:"smtp.elasticemail.com",Username:"stanislav.gupalyuk@gmail.com",Password:"590FA512160A4E2AEEDA3DB7C409E7431DEC",To:['parhomenko@tnp.com.ua','gupalyuk@tnp.com.ua'],From:"stanislav.gupalyuk@gmail.com",Subject:`Заказ (${client.value.trim()} от ${nowDate} / ${nowTime})`,Body:message.outerHTML,}).then(this.emailSend()).then(setTimeout(()=>{location.reload()},1500));}else{Swal.fire({icon:"error",title:"Неверный код!",showConfirmButton:false,timer:1500});return;}}
...
init(){$.ajax({url:"https://script.google.com/macros/s/AKfycbxbKaaRcEABRWVU3lvb77XeHfrYAyoYlo0PaObwkF5QM_O_CNw4QBaChM1Wm9LYhziz/exec",method:"get",dataType:"json",success:(data)=>{sessionStorage.setItem('reserves',JSON.stringify(data.reservs));this.reservesAll=[...data.reservs];},error:(err)=>{console.log(`Reserves: ${err}`);},});this.searchBtn.addEventListener('click',()=>this.find());document.addEventListener('keyup',event=>{if(event.code==='Enter'){let check=this.search.value;check.trim();if(!check){return;}else{this.find();}}});this.clearBtn.addEventListener('click',()=>this.clear());this.getSalesBtn.addEventListener('click',()=>this.getSalesGoods());this.newsBtn.addEventListener('click',()=>this.getNewGoods());this.filterBtn.addEventListener('click',()=>this.filtered());this.elementSection.addEventListener('click',(e)=>this.showItemInfo(e));}
