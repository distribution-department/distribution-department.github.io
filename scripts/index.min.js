...
showWeather(){$.ajax({url:"http://api.weatherapi.com/v1/current.json?key=5f6ae58e18e644888bf182855242904&q=Днепропетровск",method:"get",crossDomain:true,dataType:"json",success:(data)=>{let location=data.location.name;let temperature=data.current.temp_c;let icon=data.current.condition.icon;this.weatherCity.textContent=location.slice(0,5);this.weatherTemp.textContent=`${temperature}°C`;this.weatherImg.src=icon;this.weather.style.display='flex';},error:(err)=>{console.log(err.message)},});}}
