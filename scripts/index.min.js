...
getGoods(){this.disabledBtns();this.mainSectionBody.style.boxShadow='none';$.ajax({url:"https://script.google.com/macros/s/AKfycbzhrET853U0EziqkZygYBhb0gD8RIbTBUC1jOrMDlZBs6Ir9yLe1ksAsoDy3ucxWr7o/exec",method:"get",dataType:"json",success:(data)=>{setTimeout(()=>{this.elementSection.innerHTML='';sessionStorage.setItem('data',JSON.stringify(data.goods));sessionStorage.setItem('current','fifty');let response=data.goods.reverse();let temporaryGoods=[];for(let i=19;i>=0;i--){temporaryGoods.push(response[i])}
...
init(){this.searchBtn.addEventListener('click',()=>this.find());document.addEventListener('keyup',event=>{if(event.code==='Enter'){let check=this.search.value;check.trim();if(!check){return;}else{this.find();}}});this.clearBtn.addEventListener('click',()=>this.clear());this.newsBtn.addEventListener('click',()=>this.getNewGoods());this.filterBtn.addEventListener('click',()=>this.filtered());this.elementSection.addEventListener('click',(e)=>this.showItemInfo(e));}
