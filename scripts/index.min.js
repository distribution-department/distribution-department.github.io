...
module.exports=IndexCharts;},{}],10:[function(require,module,exports){class Login{constructor(){this.mainSection=document.querySelector('.main-section');this.cartBtn=document.querySelector('.enter-cart-btn');this.closeOrderBtn=document.querySelector('.back-cart-btn');this.formWindow=document.querySelector('.login-window');this.orderWindowBody=document.querySelector('.order-window__data');this.orderWindowProducts=document.querySelector('.order-window__products');this.bodyData=document.querySelector('.main-section__body');this.bodyMainChart=document.querySelector('.chart-all');this.bodyDonatChart=document.querySelector('.chart-new');this.clearBtn=document.querySelector('.order-clear-btn');this.calculationBtn=document.querySelector('.order-count-btn');this.sendBtn=document.querySelector('.order-send-btn');this.orderElements=null;this.allElements=null;this.choosedElements=[];this.finalProductsArr=[];this.finalSumm=null;this.finalCount=null;this.isContain=[];}
...
console.log(this.finalProductsArr);for(let i=0;i<this.finalProductsArr.length;i++){if(this.finalProductsArr[i].avlbl=='0'){this.isContain.push(this.finalProductsArr[i]);}}
...
let confirmSend=confirm("Отправить заказ?");if(confirmSend){this.telegram(this.finalProductsArr,this.finalSumm,this.finalCount,client.value,typeOfPayTelegram,clientCommentTelegram);Email.send({Host:"smtp.elasticemail.com",Username:"stanislav.gupalyuk@gmail.com",Password:"590FA512160A4E2AEEDA3DB7C409E7431DEC",To:['parhomenko@tnp.com.ua','gupalyuk@tnp.com.ua'],From:"stanislav.gupalyuk@gmail.com",Subject:`Заказ (${client.value.trim()} от ${nowDate} / ${nowTime})`,Body:message.outerHTML,}).then(M.toast({html:`Заказ отправлен!`,displayLength:1500,classes:'toast-green'})).then(this.sendBtn.classList.add('disabled')).then(setTimeout(()=>{location.reload()},1000));}}
