...
telegram(productObj,fullSumm,fullCount,client,payType,comment){const TOKEN='6746536289:AAGZlKfEqPi_t_tlzj40psKkd3PHZit1g3E';const CHAT_ID='-1002021384272';const URI_API=`https://api.telegram.org/bot${TOKEN}/sendMessage`;let requestObj=productObj;let requestClient=client;let message=`<b>Distribution</b>\n`;message+=`<b>Клиент: </b> ${requestClient}\n`;message+=`<b>Оплата: </b> ${payType}\n`;message+=`<b>Всего(шт): </b> ${fullCount}\n`;message+=`<b>Сумма(грн): </b> ${fullSumm.toFixed(2)}\n`;message+=`<b>Комментарий: </b> ${comment}\n`;message+=`-----\n`;requestObj.map(e=>createSendMessage(e));function createSendMessage(elem){message+=`<b>Код: </b> ${elem.cd}\n`;message+=`<b>Название: </b> ${elem.ttl}\n`;message+=`<b>Количество(шт): </b> ${elem.cnt}\n`;message+=`<b>Цена(грн): </b> ${elem.prc}\n`;message+=`-----\n`;}
axios.post(URI_API,{chat_id:CHAT_ID,parse_mode:'html',text:message}).then((response)=>{}).catch((error)=>{console.warn(error)}).finally(()=>{})}}
