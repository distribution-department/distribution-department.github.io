...
let confirmSend=prompt('Введите код подтверждения...');if(confirmSend==this.getConfY){this.sendBtn.classList.add('disabled');this.calculationBtn.classList.add('disabled');this.clearBtn.classList.add('disabled');this.cartBtn.classList.add('disabled');this.closeOrderBtn.classList.add('disabled');this.calcBtn.classList.add('disabled');this.telegram(this.finalProductsArr,this.finalSumm,this.finalCount,client,clientCode,typeOfPayTelegram,clientCommentTelegram);this.copyToGoogleTable(client,typeOfPayTelegram,clientCommentTelegram);Email.send({Host:"smtp.elasticemail.com",Username:"stanislav.gupalyuk@gmail.com",Password:"590FA512160A4E2AEEDA3DB7C409E7431DEC",To:['parhomenko@tnp.com.ua','gupalyuk@tnp.com.ua'],From:"stanislav.gupalyuk@gmail.com",Subject:`Заказ (${client} от ${nowDate} / ${nowTime})`,Body:message.outerHTML,}).then(this.emailSend()).then(setTimeout(()=>{location.reload()},1500));}else if(confirmSend==null){return;}else{Swal.fire({icon:"error",title:"Неверный код!",showConfirmButton:false,timer:1500});return;}}
