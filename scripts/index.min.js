...
module.exports=Calculations;},{}],4:[function(require,module,exports){class Clock{constructor(){this.init()}
...
module.exports=Clock;},{}],5:[function(require,module,exports){class Footer{constructor(element){this.footer=document.querySelector(element);}
addLicence(){let date=new Date();let year=date.toLocaleDateString().slice(6);this.footer.textContent=`© TNP Distribution 2023 - ${year}`;}}
module.exports=Footer;},{}],6:[function(require,module,exports){class IndexChartsAvailbl{constructor(){this.init();}
...
module.exports=IndexChartsAvailbl;},{}],7:[function(require,module,exports){class IndexChartsNew{constructor(){this.init();}
...
module.exports=IndexChartsNew;},{}],8:[function(require,module,exports){class IndexChartsReserved{constructor(){this.init();}
...
module.exports=IndexChartsReserved;},{}],9:[function(require,module,exports){class IndexCharts{constructor(){this.init();}
...
module.exports=IndexCharts;},{}],10:[function(require,module,exports){class Login{constructor(){this.mainSection=document.querySelector('.main-section');this.cartBtn=document.querySelector('.enter-cart-btn');this.closeOrderBtn=document.querySelector('.back-cart-btn');this.calcBtn=document.querySelector('.calculate-btn');this.formWindow=document.querySelector('.login-window');this.orderWindowBody=document.querySelector('.order-window__data');this.orderWindowProducts=document.querySelector('.order-window__products');this.bodyData=document.querySelector('.main-section__body');this.bodyMainChart=document.querySelector('.chart-all');this.bodyDonatChart=document.querySelector('.chart-new');this.sendEmailWindow=document.querySelector('.send-window');this.sendEmailText=document.querySelector('.send-window-text');this.selectNew=document.querySelector('.new-cli');this.selectOld=document.querySelector('.select-distr-client');this.clearBtn=document.querySelector('.order-clear-btn');this.calculationBtn=document.querySelector('.order-count-btn');this.sendBtn=document.querySelector('.order-send-btn');this.newClientBtn=document.querySelector('.new-client-btn');this.oldClientBtn=document.querySelector('.old-client-btn');this.progress=document.querySelector('.progress');this.orderElements=null;this.allElements=null;this.choosedElements=[];this.finalProductsArr=[];this.finalSumm=null;this.finalCount=null;this.isContain=[];this.distrClients=[];this.choosedClient='old';let today=new Date();this.getConfY=today.getFullYear();}
init(){this.allElements=JSON.parse(sessionStorage.getItem('data'));this.distrClients=JSON.parse(sessionStorage.getItem('distrClients'));this.sendBtn.classList.add('disabled');let selectDistrClient=document.querySelector('.DistrClients');let selectedOption=document.createElement('option');selectedOption.value='';selectedOption.text='Контрагент';selectedOption.selected=true;selectDistrClient.append(selectedOption);for(let i=0;i<this.distrClients.length;i++){let option=document.createElement('option');option.value=this.distrClients[i].Code;option.text=this.distrClients[i].Client;selectDistrClient.append(option);}
$('.DistrClients').formSelect();this.cartBtn.addEventListener('click',()=>this.showWindow());this.closeOrderBtn.addEventListener('click',()=>this.hideWindow());this.orderWindowBody.addEventListener('click',(e)=>this.dellElement(e));this.newClientBtn.addEventListener('click',(e)=>this.chooseNewClient());this.oldClientBtn.addEventListener('click',(e)=>this.chooseOldClient());this.sendBtn.addEventListener('click',(e)=>this.send());this.calculationBtn.addEventListener('click',(e)=>this.calculate());this.clearBtn.addEventListener('click',(e)=>this.clear());}
showWindow(){this.orderElements=JSON.parse(sessionStorage.getItem('inCart'));let productsBody=document.querySelector('.order-window__products');productsBody.innerHTML='';if(this.orderElements==0){Swal.fire({icon:"error",title:"Корзина пуста!",showConfirmButton:false,timer:1500});return}
class Product{constructor(id,article,code,name,count,priceTT,priceOpt,priceBigOpt,priceTTAA,priceOptAA,priceBigOptAA,promo,avlbl){this.id=id;this.article=article;this.code=code;this.name=name;this.count=count;this.priceTT=priceTT;this.priceOpt=priceOpt;this.priceBigOpt=priceBigOpt;this.priceTTAA=priceTTAA;this.priceOptAA=priceOptAA;this.priceBigOptAA=priceBigOptAA;this.promo=promo;this.avlbl=avlbl;}}
for(let i=0;i<this.orderElements.length;i++){for(let j=0;j<this.allElements.length;j++){if(this.allElements[j].Id==this.orderElements[i]){this.choosedElements.push(new Product(this.allElements[j].Id,this.allElements[j].Article,this.allElements[j].Code,this.allElements[j].Name,1,this.allElements[j].PriceTT,this.allElements[j].PriceOpt,this.allElements[j].PriceBigOpt,this.allElements[j].PriceTTAA,this.allElements[j].PriceOptAA,this.allElements[j].PriceBigOptAA,this.allElements[j].Promo,this.allElements[j].Available))}}}
let fullReserved=JSON.parse(sessionStorage.getItem('reserves'));let firstClientReserv='ФОП Бахурська М.О.';let secondClientReserv='ФОП Савін Ю.Б.';let thirdClientReserv='ТОВ "АШАН"';let filteredReservs=[];for(let i=0;i<fullReserved.length;i++){if(fullReserved[i].CompanyR==firstClientReserv||fullReserved[i].CompanyR==secondClientReserv||fullReserved[i].CompanyR==thirdClientReserv){filteredReservs.push(fullReserved[i]);}}
for(let i=0;i<this.choosedElements.length;i++){for(let j=0;j<filteredReservs.length;j++){if(this.choosedElements[i].code==filteredReservs[j].CodeR&&parseInt(this.choosedElements[i].avlbl)<parseInt(filteredReservs[j].CountR)){this.choosedElements[i].avlbl=filteredReservs[j].CountR;}}}
this.bodyData.style.display='none';this.bodyMainChart.style.display='none';this.bodyDonatChart.style.display='none';this.cartBtn.style.display='none';this.closeOrderBtn.style.display='block';this.formWindow.style.display='flex';this.mainSection.style.gap='10px';productsBody.append(...this.choosedElements.map(product=>this.createElements(product,this.finalProductsArr)));}
hideWindow(){this.bodyData.style.display='flex';this.bodyMainChart.style.display='block';this.bodyDonatChart.style.display='flex';this.cartBtn.style.display='block';this.closeOrderBtn.style.display='none';this.formWindow.style.display='none';this.mainSection.style.gap='30px';this.choosedElements=[];this.sendBtn.classList.add('disabled');}
chooseNewClient(){this.choosedClient='new';this.selectNew.style.display='block';this.selectOld.style.display='none';this.newClientBtn.style.display='none';this.oldClientBtn.style.display='block';}
chooseOldClient(){this.choosedClient='old';this.selectNew.style.display='none';this.selectOld.style.display='block';this.newClientBtn.style.display='block';this.oldClientBtn.style.display='none';}
createElements(product,finalProduct){let prod=document.createElement('div');prod.classList.add('product');prod.dataset.index=product.id;let checkAvailable=document.createElement('div');if(product.avlbl=='0'){checkAvailable.classList.add('unavailable');prod.append(checkAvailable);}
let inSales=document.createElement('div');if(product.promo=='1'){inSales.classList.add('insales');prod.append(inSales);}
let close=document.createElement('div');close.classList.add('product__close');let icon=document.createElement('i');icon.classList.add('small');icon.classList.add('material-icons');icon.classList.add('mat-close');icon.dataset.index=product.id;icon.append('delete_forever');close.dataset.index=product.id;close.append(icon);let photo=document.createElement('img');photo.classList.add('product__image');let checkArticle=product.article;let result;let findSlash=checkArticle.indexOf('/');if(findSlash!=-1){let check=checkArticle.split('');let final;let find=check.indexOf('/');check[find]='-';final=[...check];result=final.join('');checkArticle=result;}
photo.setAttribute('src',`./images/photo-all/images/${checkArticle}.webp`);let code=document.createElement('div');code.classList.add('product__code');code.append(product.code);let name=document.createElement('div');name.classList.add('product__name');name.append(product.name);let count=document.createElement('input');count.type='number';count.placeholder='шт...'
if(finalProduct.length==0){count.value='';}else if(finalProduct.length>0){for(let i=0;i<finalProduct.length;i++){if(finalProduct[i].ind==product.id){count.value=finalProduct[i].cnt;}}}
count.dataset.index=product.id;count.classList.add('product__count');let price=document.createElement('div');price.classList.add('product__price');if(finalProduct.length==0){price.append('₴');}else if(finalProduct.length>0){for(let i=0;i<finalProduct.length;i++){if(finalProduct[i].ind==product.id){price.append(`${finalProduct[i].prc} ₴`);if(finalProduct[i].prc=='0'){price.classList.add('zero');}}}}
prod.append(close);prod.append(photo);prod.append(code);prod.append(name);prod.append(count);prod.append(price);return prod;}
dellElement(dell){let showSumm=document.querySelector('.summ__result');let showCount=document.querySelector('.count__result');let dellIndex=null;let dellElement=dell.target;if(dellElement.matches('.product__close')||dellElement.matches('.mat-close')){dellIndex=dellElement.dataset.index;}else return;for(let i=0;i<this.choosedElements.length;i++){if(this.choosedElements[i].id==dellIndex){this.choosedElements.splice(i,1);let newChoosed=[];this.choosedElements.map(e=>newChoosed.push(e.id));if(newChoosed.length<1){this.hideWindow();}
for(let i=0;i<this.orderWindowProducts.children.length;i++){if(this.orderWindowProducts.children[i].dataset.index==dellIndex){this.orderWindowProducts.children[i].remove();}}
sessionStorage.setItem('inCart',JSON.stringify(newChoosed));}}
M.toast({html:`Товар удалён`,displayLength:1500,classes:'toast-green'});this.sendBtn.classList.add('disabled');showSumm.innerHTML='-';showCount.innerHTML='-';}
calculate(){let showSumm=document.querySelector('.summ__result');let showCount=document.querySelector('.count__result');let radios=document.querySelectorAll('input[type="radio"]');let checkedPrice=null;this.isContain=[];for(let radio of radios){if(radio.checked){checkedPrice=radio.value;}}
if(checkedPrice==null){Swal.fire({icon:"warning",title:"Выберите тип цены!",showConfirmButton:false,timer:1500});this.sendBtn.classList.add('disabled');return}
let inputCount=document.querySelectorAll('input[type="number"]');for(let num of inputCount){if(num.value<1){Swal.fire({icon:"warning",title:"Укажите везде количество!",showConfirmButton:false,timer:1500});this.sendBtn.classList.add('disabled');return}}
class CountObj{constructor(ind,cnt,prc,cd,ttl,avlbl){this.ind=ind;this.cnt=cnt;this.prc=prc;this.cd=cd;this.ttl=ttl;this.avlbl=avlbl;}}
let allCounts=document.querySelectorAll('.product__count');this.finalProductsArr=[];if(checkedPrice=='priceTT'){for(let i=0;i<allCounts.length;i++){if(this.choosedElements[i].promo=='0'){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceTT),this.choosedElements[i].code,this.choosedElements[i].name,this.choosedElements[i].avlbl));}else if(this.choosedElements[i].promo=='1'&&this.choosedElements[i].priceTTAA=='0'){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceTT),this.choosedElements[i].code,this.choosedElements[i].name,this.choosedElements[i].avlbl));}else if(this.choosedElements[i].promo=='1'&&this.choosedElements[i].priceTTAA!='0'){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceTTAA),this.choosedElements[i].code,this.choosedElements[i].name,this.choosedElements[i].avlbl));}}}else if(checkedPrice=='priceOpt'){for(let i=0;i<allCounts.length;i++){if(this.choosedElements[i].promo=='0'){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceOpt),this.choosedElements[i].code,this.choosedElements[i].name,this.choosedElements[i].avlbl));}else if(this.choosedElements[i].promo=='1'&&this.choosedElements[i].priceOptAA=='0'){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceOpt),this.choosedElements[i].code,this.choosedElements[i].name,this.choosedElements[i].avlbl));}else if(this.choosedElements[i].promo=='1'&&this.choosedElements[i].priceOptAA!='0'){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceOptAA),this.choosedElements[i].code,this.choosedElements[i].name,this.choosedElements[i].avlbl));}}}else if(checkedPrice=='priceBigOpt'){for(let i=0;i<allCounts.length;i++){if(this.choosedElements[i].promo=='0'){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceBigOpt),this.choosedElements[i].code,this.choosedElements[i].name,this.choosedElements[i].avlbl));}else if(this.choosedElements[i].promo=='1'&&this.choosedElements[i].priceBigOptAA=='0'){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceBigOpt),this.choosedElements[i].code,this.choosedElements[i].name,this.choosedElements[i].avlbl));}else if(this.choosedElements[i].promo=='1'&&this.choosedElements[i].priceBigOptAA!='0'){this.finalProductsArr.push(new CountObj(allCounts[i].dataset.index,parseInt(allCounts[i].value),parseFloat(this.choosedElements[i].priceBigOptAA),this.choosedElements[i].code,this.choosedElements[i].name,this.choosedElements[i].avlbl));}}}
for(let i=0;i<this.finalProductsArr.length;i++){if(this.finalProductsArr[i].avlbl=='0'){this.isContain.push(this.finalProductsArr[i]);}}
let productsBody=document.querySelector('.order-window__products');productsBody.innerHTML='';productsBody.append(...this.choosedElements.map(product=>this.createElements(product,this.finalProductsArr)));this.finalSumm=null;this.finalCount=null;let tempSumm=[];for(let i=0;i<this.finalProductsArr.length;i++){tempSumm.push(this.finalProductsArr[i].cnt*this.finalProductsArr[i].prc);}
this.finalSumm=tempSumm.reduce(function(sum,current){return sum+current;},0);let tempCount=[];for(let i=0;i<this.finalProductsArr.length;i++){tempCount.push(this.finalProductsArr[i].cnt);}
this.finalCount=tempCount.reduce(function(sum,current){return sum+current;},0);showSumm.innerHTML='';showCount.innerHTML='';showSumm.append(`Сумма: ${this.finalSumm.toLocaleString()} грн.`);showCount.append(`Количество: ${this.finalCount.toLocaleString()} шт.`);this.sendBtn.classList.remove('disabled');}
send(){let distrClientNew=document.querySelector('.order-client');let distrClientOld=$('.DistrClients');let client;let clientCode;let payType=$('.Paytype');let typeOfPay=document.createElement('div');let clientComment=document.createElement('div');let clientCodeDistr=document.createElement('div');let typeOfPayTelegram;let clientCommentTelegram;let comment=document.querySelector('.order-comment');let finalOrderData=[...this.finalProductsArr];let message=document.createElement('div');let table=document.createElement('table');table.setAttribute('style','border: 1px solid black; text-align: center; font-size: 0; background-color: #FFADD8E6;');let tableCaption=document.createElement('caption');tableCaption.append('');table.setAttribute('style','border: 1px solid black; text-align: center;');let tableHeadRow=document.createElement('tr');tableHeadRow.setAttribute('style','border: 1px solid black; text-align: center; font-size: 15px;');let headCode=document.createElement('th');headCode.append('Код товара');tableHeadRow.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableHeadRow.append(headCode);let headName=document.createElement('th');headName.append('Наименование');tableHeadRow.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableHeadRow.append(headName);let headCount=document.createElement('th');headCount.append('Количество');tableHeadRow.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableHeadRow.append(headCount);let headPrice=document.createElement('th');headPrice.append('Цена');tableHeadRow.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableHeadRow.append(headPrice);table.append(tableCaption);table.append(tableHeadRow);table.append(...finalOrderData.map(e=>createDataTableBody(e)));table.append(createDataTableSummary(this.finalSumm,this.finalCount));if(payType.val()=='nal'){typeOfPayTelegram=`Наличный`;typeOfPay.append(`Тип оплаты: Наличный`);}else if(payType.val()=='beznal'){typeOfPayTelegram=`Безналичный`;typeOfPay.append(`Тип оплаты: Безналичный`);}
if(comment.value.trim()==''){clientCommentTelegram=`-`;clientComment.append(`Комментарий: -`);}else{clientCommentTelegram=`${comment.value.trim()}`;clientComment.append(`Комментарий: ${comment.value.trim()}`);}
typeOfPay.setAttribute('style','margin-bottom: 10px;');clientComment.setAttribute('style','margin-bottom: 10px;');clientCodeDistr.setAttribute('style','margin-bottom: 10px;');if(this.choosedClient=='old'){clientCode=distrClientOld.val();let findClient=this.distrClients.filter(e=>e.Code==clientCode);if(findClient.length<1){Swal.fire({icon:"warning",title:"Укажите клиента!",showConfirmButton:false,timer:1500});return}else{client=findClient[0].Client;clientCodeDistr.append(`Код клиента: ${clientCode}`);}}
if(this.choosedClient=='new'){clientCode='-';client=distrClientNew.value.trim();clientCodeDistr.append(`Код клиента: ${clientCode}`);if(client==''){Swal.fire({icon:"warning",title:"Укажите клиента!",showConfirmButton:false,timer:1500});return}}
message.append(clientCodeDistr);message.append(typeOfPay);message.append(clientComment);message.append(table);let nowDate=new Date().toLocaleDateString();let nowTime=new Date().toLocaleTimeString();function createDataTableBody(info){let tableRow=document.createElement('tr');tableRow.setAttribute('style','border: 1px solid black; text-align: center; font-size: 15px;');let tableDataCode=document.createElement('td');tableDataCode.append(info.cd);tableDataCode.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableRow.append(tableDataCode);let tableDataName=document.createElement('td');tableDataName.append(info.ttl);tableDataName.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableRow.append(tableDataName);let tableDataCount=document.createElement('td');tableDataCount.append(info.cnt);tableDataCount.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableRow.append(tableDataCount);let tableDataPrice=document.createElement('td');tableDataPrice.append(info.prc.toString().replace('.',','));tableDataPrice.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px;');tableRow.append(tableDataPrice);return tableRow;}
function createDataTableSummary(summary,count){let tableRow=document.createElement('tr');tableRow.setAttribute('style','border: 1px solid black; text-align: center; font-size: 15px;');let tableDataResult=document.createElement('td');tableDataResult.append('Итого');tableDataResult.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px; font-weight: bold;');tableDataResult.setAttribute('colspan','2');tableRow.append(tableDataResult);let tableDataCountResult=document.createElement('td');tableDataCountResult.append(count);tableDataCountResult.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px; font-weight: bold;');tableRow.append(tableDataCountResult);let tableDataPriceResult=document.createElement('td');tableDataPriceResult.append(summary.toFixed(2).toString().replace('.',','));tableDataPriceResult.setAttribute('style','border: 1px solid black; text-align: center; padding: 5px; font-weight: bold;');tableRow.append(tableDataPriceResult);return tableRow;}
let chechFree=this.finalProductsArr.filter(elem=>elem.prc=='0');if(chechFree.length>0){Swal.fire({icon:"warning",title:"Есть товар без цены!",showConfirmButton:false,timer:1500});return}
if(this.isContain.length>0){Swal.fire({icon:"warning",title:"Есть товар без остатка!",showConfirmButton:false,timer:1500});return}
if(payType.val()==''){Swal.fire({icon:"warning",title:"Не указан тип оплаты!",showConfirmButton:false,timer:1500});return}
let confirmSend=prompt('Введите код подтверждения...');if(confirmSend==this.getConfY){this.sendBtn.classList.add('disabled');this.calculationBtn.classList.add('disabled');this.clearBtn.classList.add('disabled');this.cartBtn.classList.add('disabled');this.closeOrderBtn.classList.add('disabled');this.calcBtn.classList.add('disabled');this.progress.style.display='block';let checkSendMessage=null;let emailSendData={service_id:'service_0tqqyh9',template_id:'template_1ldy9z4',user_id:'fImwOFelNDvpciMl4',template_params:{'to':'parhomenko@tnp.com.ua','from':'TNP_Distribution','subject':`Заказ (${client} от ${nowDate} | ${nowTime})`,'message':message.outerHTML}};$.ajax('https://api.emailjs.com/api/v1.0/email/send',{type:'POST',data:JSON.stringify(emailSendData),contentType:'application/json'}).done(()=>{checkSendMessage='Успешно';this.progress.style.display='none';Swal.fire({icon:"success",title:"Заказ отправлен!",showConfirmButton:false,timer:2000});setTimeout(()=>{location.reload()},2000);}).fail(error=>{checkSendMessage='Ошибка';this.progress.style.display='none';Swal.fire({icon:"error",title:'Ошибка! Заказ не отправлен!',showConfirmButton:false,timer:2000});this.sendBtn.classList.remove('disabled');this.calculationBtn.classList.remove('disabled');this.clearBtn.classList.remove('disabled');this.cartBtn.classList.remove('disabled');this.closeOrderBtn.classList.remove('disabled');this.calcBtn.classList.remove('disabled');return;}).always(()=>{this.telegram(this.finalProductsArr,this.finalSumm,this.finalCount,client,clientCode,typeOfPayTelegram,clientCommentTelegram,checkSendMessage);this.copyToGoogleTable(client,typeOfPayTelegram,clientCommentTelegram);});}else if(confirmSend==null){return;}else{Swal.fire({icon:"error",title:"Неверный код!",showConfirmButton:false,timer:1500});return;}}
clear(){Swal.fire({title:"Очистить всё?",icon:"question",showCancelButton:true,confirmButtonColor:"#3085d6",cancelButtonColor:"#d33",confirmButtonText:"Да, очистить!",cancelButtonText:"Отмена"}).then((result)=>{if(result.isConfirmed){this.sendBtn.classList.add('disabled');this.calculationBtn.classList.add('disabled');this.clearBtn.classList.add('disabled');this.cartBtn.classList.add('disabled');this.closeOrderBtn.classList.add('disabled');this.calcBtn.classList.add('disabled');sessionStorage.setItem('inCart',[0]);location.reload();}});}
emailSend(){Swal.fire({icon:"success",title:"Заказ отправлен!",showConfirmButton:false,timer:1500});}
copyToGoogleTable(client,payType,comment){let nowDate=new Date().toLocaleDateString();let nowTime=new Date().toLocaleTimeString();let summ=this.finalSumm.toFixed(2).toString().replace('.',',');let count=this.finalCount;let finalOrderData=[...this.finalProductsArr];let resultArr=[];for(let i=0;i<finalOrderData.length;i++){resultArr.push(`Код: ${finalOrderData[i].cd}, Название: ${finalOrderData[i].ttl}, Цена: ${finalOrderData[i].prc}, Количество: ${finalOrderData[i].cnt}`)}
let resultStringData=resultArr.join(";\n");let sendData={Data:nowDate,Time:nowTime,Client:client,PaymentType:payType,OrderData:resultStringData,CountAll:count,Summ:summ,Comment:comment}
$.ajax({url:"https://script.google.com/macros/s/AKfycbxooCJ88qYs1U_6eeuLuTXqCMzA30y5fzGScTeXmC7TuqIkJRrVSFBPhKxspUANEtT1-A/exec",type:"POST",crossDomain:true,data:JSON.stringify(sendData),dataType:"json",success:(response)=>{},error:(error)=>{}});}
telegram(productObj,fullSumm,fullCount,client,clCode,payType,comment,check){const TOKEN='6746536289:AAGZlKfEqPi_t_tlzj40psKkd3PHZit1g3E';const CHAT_ID='-1002021384272';const URI_API=`https://api.telegram.org/bot${TOKEN}/sendMessage`;let requestObj=productObj;let requestClient=client;let requestClientCode=clCode;let requestSendStatus=check;let text=`<b>Distribution</b>\n`;text+=`<b>Статус: </b> ${requestSendStatus}\n`;text+=`<b>Большой заказ</b>\n`;text+=`<b>Клиент: </b> ${requestClient}\n`;text+=`<b>Код клиента: </b> ${requestClientCode}\n`;text+=`<b>Оплата: </b> ${payType}\n`;text+=`<b>Всего(шт): </b> ${fullCount}\n`;text+=`<b>Сумма(грн): </b> ${fullSumm.toFixed(2)}\n`;text+=`<b>Комментарий: </b> ${comment}\n`;let message=`<b>Distribution</b>\n`;message+=`<b>Статус: </b> ${requestSendStatus}\n`;message+=`<b>Клиент: </b> ${requestClient}\n`;message+=`<b>Код клиента: </b> ${requestClientCode}\n`;message+=`<b>Оплата: </b> ${payType}\n`;message+=`<b>Всего(шт): </b> ${fullCount}\n`;message+=`<b>Сумма(грн): </b> ${fullSumm.toFixed(2)}\n`;message+=`<b>Комментарий: </b> ${comment}\n`;message+=`-----\n`;requestObj.map(e=>createSendMessage(e));function createSendMessage(elem){message+=`<b>Код: </b> ${elem.cd}\n`;message+=`<b>Название: </b> ${elem.ttl}\n`;message+=`<b>Количество(шт): </b> ${elem.cnt}\n`;message+=`<b>Цена(грн): </b> ${elem.prc}\n`;message+=`-----\n`;}
if(JSON.stringify(message).length>4000){axios.post(URI_API,{chat_id:CHAT_ID,parse_mode:'html',text:text}).then((response)=>{}).catch((error)=>{console.warn(error)}).finally(()=>{})}else{axios.post(URI_API,{chat_id:CHAT_ID,parse_mode:'html',text:message}).then((response)=>{}).catch((error)=>{console.warn(error)}).finally(()=>{})}}}
module.exports=Login;},{}],11:[function(require,module,exports){let IndexCharts=require('./index-charts');let IndexChartsNew=require('./index-charts-new');let IndexChartsAvailbl=require('./index-charts-availbl');let IndexChartsReserved=require('./index-charts-reserved');let AddToCart=require('./add-to-cart');let Login=require('./login');let ScalePhoto=require('./scale-photo');class RenderGoods{constructor(){this.container=document.querySelector('.main-section__body-container');this.mainSectionBody=document.querySelector('.main-section__body');this.elementSection=document.querySelector('.main-section__body-data');this.updata=document.querySelector('.updata');this.fullInfo=document.querySelector('.main-section__body-full-info');this.loader=document.querySelector('.main-section__body-loader');this.notFound=document.querySelector('.main-section__body-not-found');this.searchBtn=document.querySelector('.search-btn');this.filterBtn=document.querySelector('.filter-btn');this.newsBtn=document.querySelector('.news-btn');this.getSalesBtn=document.querySelector('.sales-btn');this.clearBtn=document.querySelector('.clear-btn');this.getAllBtn=document.querySelector('.get-all-btn');this.logInBtn=document.querySelector('.login-btn');this.showCalcBnt=document.querySelector('.calculate-btn');this.sampleLoader=document.querySelector('.loader');this.choosedGoods=[];}
init(){this.getAllBtn.addEventListener('click',()=>this.getAllGoods());}
getGoods(){this.disabledBtns();this.mainSectionBody.style.boxShadow='none';$.ajax({url:"https://script.google.com/macros/s/AKfycbwhly1tavDuSDl-Bpsd6vR_VsbYWD7f1DSthUEE2o4G5om_XJSPeIiCQ-uZYzZ_Mrwk/exec",method:"get",crossDomain:true,dataType:"json",success:(data)=>{setTimeout(()=>{this.elementSection.innerHTML='';sessionStorage.setItem('data',JSON.stringify(data.goods));sessionStorage.setItem('current','fifty');let response=data.goods.reverse();let temporaryGoods=[];for(let i=19;i>=0;i--){temporaryGoods.push(response[i])}
...
disabledBtns(){this.searchBtn.classList.add('disabled');this.filterBtn.classList.add('disabled');this.newsBtn.classList.add('disabled');this.clearBtn.classList.add('disabled');this.getAllBtn.classList.add('disabled');this.getSalesBtn.classList.add('disabled');}
removeDisabledBtns(){this.searchBtn.classList.remove('disabled');this.filterBtn.classList.remove('disabled');this.newsBtn.classList.remove('disabled');this.clearBtn.classList.remove('disabled');this.getAllBtn.classList.remove('disabled');this.getSalesBtn.classList.remove('disabled');}}
module.exports=RenderGoods;},{"./add-to-cart":2,"./index-charts":9,"./index-charts-availbl":6,"./index-charts-new":7,"./index-charts-reserved":8,"./login":10,"./scale-photo":12}],12:[function(require,module,exports){class ScalePhoto{constructor(){this.body=document.querySelector('body');this.scalePhotoContainer=document.querySelector('.scale-photo-container');this.scalePhoto=document.querySelector('.scale-img');this.scalePhotoClose=document.querySelector('.scale-photo-close');this.scalePhotoDownload=document.querySelector('.dwnld-img');this.photoAttribute=null;this.init();}
...
module.exports=ScalePhoto;},{}],13:[function(require,module,exports){class Searching{constructor(){this.container=document.querySelector('.main-section__body-container');this.elementSection=document.querySelector('.main-section__body-data');this.updata=document.querySelector('.updata');this.searchBtn=document.querySelector('.search-btn');this.filterBtn=document.querySelector('.filter-btn');this.newsBtn=document.querySelector('.news-btn');this.clearBtn=document.querySelector('.clear-btn');this.getAllBtn=document.querySelector('.get-all-btn');this.getSalesBtn=document.querySelector('.sales-btn');this.sampleLoader=document.querySelector('.loader');this.loader=document.querySelector('.main-section__body-loader');this.fullInfo=document.querySelector('.main-section__body-full-info');this.notFound=document.querySelector('.main-section__body-not-found');this.search=document.querySelector('.search-input');this.findTempArr=[];this.searchArr=[];this.findCount=null;this.singleElem=null;this.choosedGoods=[];this.reservesAll=[];this.findTextData=[];this.init();}
init(){$.ajax({url:"https://script.google.com/macros/s/AKfycbxbKaaRcEABRWVU3lvb77XeHfrYAyoYlo0PaObwkF5QM_O_CNw4QBaChM1Wm9LYhziz/exec",method:"get",crossDomain:true,dataType:"json",success:(data)=>{sessionStorage.setItem('reserves',JSON.stringify(data.reservs));this.reservesAll=[...data.reservs];},error:(err)=>{console.log(`Reserves: ${err}`);},});$.ajax({url:"https://script.google.com/macros/s/AKfycbwbQu7KsI_kTPp-y9Jvbt19rOoS2yRrqHq6T7Zjqnvdv4zCUEYwuqR-vR2V2DxmTwW9/exec",method:"get",crossDomain:true,dataType:"json",success:(data)=>{sessionStorage.setItem('distrClients',JSON.stringify(data.clients));},error:(err)=>{console.log(`DistrClients: ${err}`);},});this.searchBtn.addEventListener('click',()=>this.find());document.addEventListener('keyup',event=>{if(event.code==='Enter'){let check=this.search.value;check.trim();if(!check){return;}else{this.find();}}});this.clearBtn.addEventListener('click',()=>this.clear());this.getSalesBtn.addEventListener('click',()=>this.getSalesGoods());this.newsBtn.addEventListener('click',()=>this.getNewGoods());this.filterBtn.addEventListener('click',()=>this.filtered());this.elementSection.addEventListener('click',(e)=>this.showItemInfo(e));}
...
this.search.value='';},200);return;}}
this.fullInfo.append(this.createSingleElement(find[0]));let summData=document.querySelector('.summary-data');let summDataIndex=summData.dataset.index;let personalCart=document.querySelector('.item__cart-summary');personalCart.addEventListener('click',()=>{let allInCart=JSON.parse(sessionStorage.getItem('inCart'));if(allInCart=='0'){allInCart=[summDataIndex]}
allInCart.push(summDataIndex);let filteredGoods=[...new Set(allInCart)];sessionStorage.setItem('inCart',JSON.stringify(filteredGoods));M.toast({html:`Товар добавлен`,displayLength:1500,classes:'toast-yellow'});personalCart.style.opacity='0';})
setTimeout(()=>{this.loader.style.display='none';this.elementSection.style.display='none';this.updata.style.display='none';this.fullInfo.style.display='flex';this.notFound.style.display='none'
...
this.removeDisabledBtns();this.search.value='';},200)}
getSalesGoods(){sessionStorage.setItem('current','sales')
this.disabledBtns();this.updata.style.display='none';this.elementSection.innerHTML='';this.loader.style.display='flex';this.elementSection.style.display='none';this.fullInfo.style.display='none';this.notFound.style.display='none';this.sampleLoader.style.display='block';let temporaryArr=[...JSON.parse(sessionStorage.getItem('data'))];let find=[];find.push(temporaryArr.filter(elem=>elem.Promo=='1'));let flatted=find.flat(1);flatted.sort((a,b)=>a.Name>b.Name?1:-1);this.elementSection.append(...flatted.map(e=>this.createElements(e)));M.toast({html:`Найдено ${flatted.length} товаров`,displayLength:1500,classes:'toast'});setTimeout(()=>{this.updata.style.display='block';this.loader.style.display='none';this.elementSection.style.display='block';this.notFound.style.display='none'
this.removeDisabledBtns();this.search.value='';},200)}
...
this.removeDisabledBtns();this.search.value='';$('.collapsible').collapsible();},100);let personalCart=document.querySelector('.item__cart-summary');personalCart.addEventListener('click',()=>{let allInCart=JSON.parse(sessionStorage.getItem('inCart'));if(allInCart=='0'){allInCart=[currentIndex]}
allInCart.push(currentIndex);let filteredGoods=[...new Set(allInCart)];sessionStorage.setItem('inCart',JSON.stringify(filteredGoods));M.toast({html:`Товар добавлен`,displayLength:1500,classes:'toast-yellow'});personalCart.style.opacity='0';})}
createElements(data){this.choosedGoods=JSON.parse(sessionStorage.getItem('inCart'));let elementID=data.Id;let element=document.createElement('div');element.classList.add('item');element.dataset.index=data.Id;let cart=document.createElement('div');cart.classList.add('item__cart');let logo=document.createElement('div');logo.classList.add('item__logo');let img=document.createElement('img');let checkArticle=data.Article;let result;let findSlash=checkArticle.indexOf('/');if(findSlash!=-1){let check=checkArticle.split('');let final;let find=check.indexOf('/');check[find]='-';final=[...check];result=final.join('');checkArticle=result;}
img.setAttribute('src',`./images/photo-all/images/${checkArticle}.webp`);logo.append(img);element.append(logo)
let itemCode=document.createElement('div');itemCode.classList.add('item__code');let code=document.createElement('div');code.classList.add('code');code.append(data.Code);itemCode.append(code);element.append(itemCode);let itemArticle=document.createElement('div');itemArticle.classList.add('item__article');let article=document.createElement('div');article.classList.add('article');article.append(data.Article);itemArticle.append(article);element.append(itemArticle);let itemName=document.createElement('div');itemName.classList.add('item__title');let name=document.createElement('div');name.classList.add('title');name.append(data.Name);itemName.append(name);element.append(itemName);element.append(cart);for(let i=0;i<this.choosedGoods.length;i++){if(this.choosedGoods[i]==elementID){cart.style.opacity='0';}}
let status=document.createElement('div');status.classList.add('item__status');if(data.Status==='new'){element.append(status);}
let available=document.createElement('div');available.classList.add('item__available');available.innerText=data.Available;if(parseInt(data.Available)>0){available.classList.add('av_green');}else if(parseInt(data.Available)==0){available.classList.add('av_orange');}
...
collapsElemLi.append(collapsElemLiHead);collapsElemLi.append(collapsElemLiBody);collapsElem.append(collapsElemLi);body.append(section);body.append(cart);body.append(collapsElem);finaly.append(newPos);finaly.append(header);finaly.append(body);this.singleElem=1;return finaly;}
disabledBtns(){this.container.style.backgroundImage='linear-gradient(to bottom, snow, snow)';this.searchBtn.classList.add('disabled');this.filterBtn.classList.add('disabled');this.newsBtn.classList.add('disabled');this.clearBtn.classList.add('disabled');this.getAllBtn.classList.add('disabled');this.getSalesBtn.classList.add('disabled');}
removeDisabledBtns(){this.container.style.backgroundImage='linear-gradient(to bottom, #f3f4fa, #e9edff)';this.searchBtn.classList.remove('disabled');this.filterBtn.classList.remove('disabled');this.newsBtn.classList.remove('disabled');this.clearBtn.classList.remove('disabled');this.getAllBtn.classList.remove('disabled');this.getSalesBtn.classList.remove('disabled');}}
module.exports=Searching;},{}],14:[function(require,module,exports){class ShowAsside{constructor(){this.showAssideBtn=document.querySelector('.show-asside');this.show=null;this.init();}
...
this.weatherImg.src=icon;this.weather.style.display='flex';},error:(err)=>{console.log(err.message)},});$.ajax({url:"https://bank.gov.ua/NBUStatService/v1/statdirectory/exchange?json",method:"GET",crossDomain:true,dataType:"json",success:(data)=>{let usd=data.filter(f=>f.txt=='Долар США');let resultUsdInfo=usd[0].rate;this.currencyInfo.textContent=resultUsdInfo;},error:(err)=>{console.log(err.message);},});}}
