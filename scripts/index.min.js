...
let confirmSend=prompt('Введите код подтверждения...');if(confirmSend==this.getConfY){this.sendBtn.classList.add('disabled');this.calculationBtn.classList.add('disabled');this.clearBtn.classList.add('disabled');this.cartBtn.classList.add('disabled');this.closeOrderBtn.classList.add('disabled');this.calcBtn.classList.add('disabled');let checkSendMessage=null;let emailSendData={service_id:'service_0tqqyh9',template_id:'template_1ldy9z4',user_id:'fImwOFelNDvpciMl4',template_params:{'to':'parhomenko@tnp.com.ua','copy':'gupalyuk@tnp.com.ua','from':'TNP_Distribution','subject':`Заказ (${client} от ${nowDate} | ${nowTime})`,'message':message.outerHTML}};$.ajax('https://api.emailjs.com/api/v1.0/email/send',{type:'POST',data:JSON.stringify(emailSendData),contentType:'application/json'}).done(()=>{checkSendMessage='Успешно';Swal.fire({icon:"success",title:"Заказ отправлен!",showConfirmButton:false,timer:2000});setTimeout(()=>{location.reload()},2000);}).fail(error=>{checkSendMessage='Ошибка';Swal.fire({icon:"error",title:'Ошибка! Заказ не отправлен!',showConfirmButton:false,timer:2000});this.sendBtn.classList.remove('disabled');this.calculationBtn.classList.remove('disabled');this.clearBtn.classList.remove('disabled');this.cartBtn.classList.remove('disabled');this.closeOrderBtn.classList.remove('disabled');this.calcBtn.classList.remove('disabled');return;}).always(()=>{this.telegram(this.finalProductsArr,this.finalSumm,this.finalCount,client,clientCode,typeOfPayTelegram,clientCommentTelegram,checkSendMessage);this.copyToGoogleTable(client,typeOfPayTelegram,clientCommentTelegram);});}else if(confirmSend==null){return;}else{Swal.fire({icon:"error",title:"Неверный код!",showConfirmButton:false,timer:1500});return;}}
